...
def read_all_tasks() -> list[TaskWithID]:
    """Retrieve all the tasks from a .csv file"""
...
def get_next_id():
    """Retrieves the new ID based on the existing ones in the database"""
    try:
...
def write_task_into_csv(
    ...
    """write the task with the ID in the CSV file:"""
    with open(
...
def modify_task(id: int, task: dict) -> Optional[TaskWithID]:
    ...
    with open(DATABASE_FILENAME, mode="w", newline="") as csvfile:
        """The function to modify the task"""
        writer = csv.DictWriter(
            csvfile,
            fieldnames=column_fields
        )
...
def remove_task(id:int) -> bool:
    """Remove the task with a specific id."""
    deleted_task: Optional[Task] = None
    tasks = read_all_tasks()

    with open(DATABASE_FILENAME, mod="w", newline="") as csvfile:
        ...
        for task in tasks:
            if task.id == id:
                deleted_task = task
                continue
            writer.writerow(task.model.dump)
